...
func _ready() -> void:
	...
	spit_earned.text = str(ScoreHolder.final_spit)
	cust_seen.text = str(ScoreHolder.cust_seen)
	perf_bonus.text = str(ScoreHolder.perf_bonus)
	
	final_score.text = str(ScoreHolder.final_spit + ScoreHolder.cust_seen + ScoreHolder.perf_bonus)
